...
class _ShareholderHomePageState extends State<ShareholderHomePage> {
    ...
    switch (period) {
      ...
      case 'week':
        for (int i = 0; i < 7; i++) {
          ...
          data.add({
            "time": DateFormat('E').format(now.add(Duration(days: i))), // "Mon", "Tue"...
            "value": formatValue(baseValue)
          });
        }
        break;
      ...
      case 'month':
        for (int i = 0; i < 30; i++) {
          ...
          data.add({
            "time": DateFormat('MMM dd').format(now.add(Duration(days: i))), // "Jan 15"
            "value": formatValue(baseValue)
          });
        }
        break;
      ...
      case 'all':
        for (int i = 0; i < 12; i++) {
          ...
          data.add({
            "time": DateFormat('MMM').format(now.add(Duration(days: i * 30))), // "Jan", "Feb"
            "value": formatValue(baseValue)
          });
        }
        break;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Container(
        color: Theme.of(context).colorScheme.primary, // Ensures SafeArea blends with content
        child: SafeArea(
          child: pages[_selectedIndex],
        ),
      ),
     /* floatingActionButton: FloatingActionButton(
        onPressed: () => _showSnackBar('Quick action triggered'),
  ...
  Widget _buildDashboard() {
    return SingleChildScrollView(
      child: Column(
        children: [
          // Top section with unified color
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                QuickActionsComponent(
                  onHistoryTap: () => _showSnackBar('View History tapped'),
                  onKYCTap: () => _showSnackBar('Upload KYC tapped'),
                  onSupportTap: () => _showSnackBar('Contact Support tapped'),
                ),
              ],
            ),
          ),
